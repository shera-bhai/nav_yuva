...
export default function Loading() {
  ...
  // Enforce minimum display time of 10 seconds
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsMinTimeElapsed(true);
    }, 10000); // 10 seconds minimum display time
  ...
  // Only render the loading screen if the minimum time hasn't elapsed or progress isn't complete
  if (progress < 100 || !isMinTimeElapsed) {
    return (
      <>
      <ParticleBackground/>
      <div className="fixed inset-0 bg-gradient-to-r from-primary/10 to-secondary/10 flex flex-col items-center justify-center z-50">
        <div className="w-full max-w-md px-4 flex flex-col items-center">
          ...
          <p className="text-gray-600 text-sm font-medium" aria-live="polite">
            {progress < 100 || !isMinTimeElapsed ? 'Loading...' : 'Ready!'}
          </p>
        </div>
      </div>
      </>
  ...
  return null; // Hide the loading screen after the minimum time and progress completion
}